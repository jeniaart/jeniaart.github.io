...
// Get CSV file path from meta tag and convert to full URL
const csvMetaPath = document.querySelector('meta[name="csv-path"]').content;
const csvFilePath = new URL(csvMetaPath, window.location.href).href;
...
async function loadCSVData() {
    try {
        ...
        tbody.innerHTML = '<tr><td colspan="100%" class="text-center py-4">Loading data...</td></tr>';
            ...
            chunk: function(results, parser) {
                ...
                if (results.data && results.data.length > 0) {
                    if (tableData.length === 0) {
                        // First chunk - setup headers
                        headers = Object.keys(results.data[0]);
                        createTableHeaders(headers);
                    }
                    
                    // Map and append the chunk data
                    const chunkData = results.data.map(row => 
                        headers.map(header => row[header] || '')
                    );
                    tableData = tableData.concat(chunkData);
...
// Add debouncing to filterData to improve search performance
let filterTimeout;
